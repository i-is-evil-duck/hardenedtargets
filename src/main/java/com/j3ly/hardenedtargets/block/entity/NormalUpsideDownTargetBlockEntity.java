...
public class NormalUpsideDownTargetBlockEntity extends BlockEntity implements Nameable {
    public float rot;
    public float oRot;
    ...
    public NormalUpsideDownTargetBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.NORMAL_UPSIDE_DOWN_TARGET_BE.get(), pos, state);
    }

    public static void clientTick(Level level, BlockPos pos, BlockState state, NormalUpsideDownTargetBlockEntity be) {
        be.oRot = be.rot;
    ...
    public void hit(Level level, BlockState state, BlockHitResult hitResult, boolean isUpper) {
        ...
        level.setBlock(hitPos,
                state.setValue(TargetBlock.STAND, false)
                        .setValue(TargetBlock.OUTPUT_POWER, strength),
                3);
        level.scheduleTick(hitPos, state.getBlock(), RESET_TIME);

        float pitch = 0.9f + level.random.nextFloat() * 0.1f;
        level.playSound(null, hitPos, ModSounds.TARGET_HIT.get(), SoundSource.BLOCKS, 8.0f, pitch);
    }
    ...
    public Component getName() {
        return customName != null ? customName : Component.translatable("block.hardenedtargets.normal_upside_down_target");
    }
